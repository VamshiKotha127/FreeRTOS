...
int main(void)
{
  ...
  xSerialSemaphore = xSemaphoreCreateCounting(1,0);//initial count =0, max count = 1

  xTaskCreate(digital_sensor_task, "ButtonRead", 128, NULL, 2, NULL);
  xTaskCreate(analog_sensor_task, "SensorRead", 128, NULL, 1, NULL);


  xSemaphoreGive(xSerialSemaphore); //initially give semaphore. Here if we make the initial count =1, then this line is not needed
  /* Init scheduler */
  vTaskStartScheduler();
...
void digital_sensor_task(void *pvParameters)
{
	...
	while(1)
	{
		  ...
		  // if the semaphore is not available, wait for 5 ticks of the scheduler to see if it becomes free
