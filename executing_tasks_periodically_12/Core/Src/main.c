...
void vBlueLedControllerTask(void *pvParameters)
{
	...
	const TickType_t xPeriod = pdMS_TO_TICKS(500); //500ms

	xLastWakeTime	= xTaskGetTickCount(); //last wake timestamp

	while(1)
	{
		vTaskDelayUntil(&xLastWakeTime,xPeriod); //this task executes every 500ms
		//previous wake time gets updated automatically after this function call-->vTaskDelayUntil every time
		//thats why we have to pass the address of last wake time
		BlueTaskProfiler++;
	}
}
