...
//
//we can use event groups for synchronising tasks and for broadcasting events to multiple tasks
//
...
static void vEventBitReadingTask(void *pvParameters)
{
	...
	while(1)
	{
		// clear on exit --> pdTRUE, WaitForALLBits --> pdFALSE
		xEventGroupValue = xEventGroupWaitBits(xEventGroup,xBitsWaitFor, pdTRUE, pdFALSE, portMAX_DELAY);
